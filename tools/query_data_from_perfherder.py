...
class QueryData(object):
    ...
    def query_signatures(self):
        url_str = API_URL_QUERY_SIGNATURE_LIST % (DEFAULT_PERFHERDER_STAGE_URL, PROJECT_NAME_MOZILLA_CENTRAL, str(DEFAULT_HASAL_FRAMEWORK_NO))
    ...
    def query_data(self, query_interval, query_keyword, query_btype, query_platform, query_suite_name, query_begin_date, query_end_date):
        ...
        for signature in signature_list:
            url_str = API_URL_QUERY_DATA % (DEFAULT_PERFHERDER_STAGE_URL, PROJECT_NAME_MOZILLA_CENTRAL, str(DEFAULT_HASAL_FRAMEWORK_NO), str(query_interval), signature)
            query_obj = self.send_url_data(url_str)
            if query_obj:
                json_obj = json.loads(query_obj.read())
